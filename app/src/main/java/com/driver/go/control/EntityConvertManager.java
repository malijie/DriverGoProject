...
public class EntityConvertManager {
    ...
    public static ExamRecord getExamRecordEntity(Cursor  cursor){
        ExamRecord examRecordItem = new ExamRecord();
        examRecordItem.setScore(cursor.getInt(cursor.getColumnIndex(cursor.getColumnName(0))));
        examRecordItem.setDate(cursor.getString(cursor.getColumnIndex(cursor.getColumnName(1))));
        return examRecordItem;
    }
}
